...
var config =  {
    ...
    isv: false, //是否显示版本信息
    istimestamp : true,//是否以时间戳提交
    isphp : true//是否以PHP格式显示
};
...
Dates.query = function(node){
    if(node && node.nodeType === 1){
        if(node.tagName.toLowerCase() !== 'input'){
            throw new Error(lang.errorelem);
        }
        return node;
    }
...
Dates.check = function(){
    ...
    if(value.replace(/\s/g, '') !== ''){
        if(!exp.test(value)){
            Dates.elem[as.elemv] = '';
            Dates.msg(lang.errorformat);
            return 1;
        } else if(isvoid[0]){
            Dates.elem[as.elemv] = '';
            Dates.msg(lang.errorrange);
            return 1;
        } else {
...
Dates.viewDate = function(Y, M, D){
    ...
    //锁定年月
    as.year.value = Dates.ymd[0] + lang.year;
    as.month.value = Dates.digit(Dates.ymd[1] + 1) + lang.month;
...
Dates.viewYears = function(YY){
    ...
    Dates.each(new Array(14), function(i){
        if(i === 7) {
            str += '<li '+ (parseInt(as.year.value) === YY ? 'class="'+ as[2] +'"' : '') +' y="'+ YY +'">'+ YY + lang.year +'</li>';
        } else {
            str += '<li y="'+ (YY-7+i) +'">'+ (YY-7+i) + lang.year + '</li>';
        }
    }); 
...
Dates.timestamp = function(elem,options){
    ...
    if(istimestamp){
        ...
        elt.name = elem.name;
        elt.type = 'hidden';
        elt.className = 'fl_timestamp '+elem.id;
        elt.value = elem.value;
        elem.name = '';
        var isphp = typeof(Dates.options.isphp) == 'undefined' ? config.isphp : Dates.options.isphp;
        if(isphp){
            if(elem.value == ''){
...
Dates.view = function(elem, options){
    ...
    if(!Dates.box){
          ...
          +'<div class="laydate_ym laydate_m" id="laydate_MM">'
            ...
            +'<div class="laydate_yms" id="laydate_ms">'+ function(){
                ...
                Dates.each(new Array(12), function(i){
                    str += '<span m="'+ i +'">'+ Dates.digit(i+1) + lang.month + '</span>';
                });
                return str;
            }() +'</div>'
          +'</div>'
        ...
        +'<div class="laydate_bottom">'
          +'<ul id="laydate_hms">'
            +'<li class="laydate_sj">'+ lang.time +'</li>'
          ...
          +'<div class="laydate_btn">'
            +'<a id="laydate_clear">'+ lang.clear +'</a>'
            +'<a id="laydate_today">'+ lang.today +'</a>'
            +'<a id="laydate_ok">'+ lang.ok +'</a>'
          +'</div>'
...
Dates.creation = function(ymd, hide){
    ...
    if(isphp){
        ...
        // Dates.elem[as.elemv] = 'hehe';
    }else{
        Dates.elem[as.elemv] = getDates;
    }
    var istimestamp = typeof(Dates.options.istimestamp) == 'undefined' ? config.istimestamp : Dates.options.istimestamp;
    if(istimestamp){
        var client = new hprose.HttpClient("http://www.itsky71.me/Server", ['mktime']);
        client.mktime(
            ymd[0],
            ymd[1],
            ymd[2],
            ...
            function(result) {
                var resobj = JSON.parse(result);
                var stamp = document.getElementsByClassName('fl_timestamp '+resobj.elem)[0];
                stamp.value = resobj.timestamp == 'false' ? '' : resobj.timestamp;
            }, function(name, err) {
                throw new Error(err);
            }
        );
...
Dates.events = function(){
    ...
    log.hmss = [lang.hours, lang.minutes, lang.seconds];
    log.hmsarr = [];
    
    //生成时分秒或警告信息
    Dates.msg = function(i, title){
        var str = '<div class="laydte_hsmtex">'+ (title || lang.cue) +'<span>×</span></div>';
        if(typeof i === 'string'){
            str += '<p>'+ i +'</p>';
            Dates.shde(S('#'+as[0]));
